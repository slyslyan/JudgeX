      ...
      <nav class="px-2">
        ...
        >
          <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" v-if="item.icon.includes('M16 7a4')"/>
            <path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" v-if="item.icon.includes('10.325')"/>
            <path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" v-if="item.icon.includes('5.882V19.24')"/>
          </svg>
          {{ item.label }}
        </button>
      </nav>
